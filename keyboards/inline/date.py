...
def calendar():
    ...
    calendar_2, step = MyTranslationCalendar(min_date=today, max_date=datetime.date(next_year, 12, 31)).build()
    return calendar_2
...
def calendar_1(res_date):
    ...
    calendar_3, step_1 = MyTranslationCalendar(min_date=datetime.date(int(dates[0]), int(dates[1]), int(dates[2]) + 1),
